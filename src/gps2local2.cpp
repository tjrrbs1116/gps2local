...
#define gps_limit_cov 0.0004
#define limit_dt 0.025
  ...
  geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg)
{
    ...
    if( x_cov+y_cov < limit_cov ){
        ...
        if(g_status==2 && gps_x_cov+gps_y_cov < gps_limit_cov && fabs(dt)< limit_dt)
        {
    ...
    void gps2local::maketransform(){
        ...
        for(int i=0; i<3; i++){
            ...
            for(int j=0; j<3; j++)
            {   
                if (j ==0){map_data[i,j] = temps_.map_data->pose.pose.position.x; gps[i][j] = temps_.gps_data->longitude; }
                if (j ==1){map_data[i,j] = temps_.map_data->pose.pose.position.y; gps[i][j] = temps_.gps_data->latitude;}
        ...
        this->make_transform_ok =true;
            ...
            gps_params[1] = output(0,1);
            gps_params[2] = output(1,0);
            gps_params[3] = output(1,1);
            gps_params[4] = output(2,0);
